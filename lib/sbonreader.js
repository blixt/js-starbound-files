...
SbonReader.prototype.readIntVar = function () {
  ...
  if (value instanceof BigInt) {
    if (value.and(1) == 1) value = value.negate();
...
SbonReader.prototype.readUintVar = function () {
  ...
  while (true) {
    ...
    if (value > 0x2000000) {
      // We can't perform bitwise operations on >32-bit numbers, so switch to
      // BigInt mode once we get close to that limit.
      return this._readBigInt(value);
    }
  }
};
...
SbonReader.prototype._readBigInt = function (value) {
  // Construct a list of 7-bit integers.
  var items = [];

  while (value) {
    items.unshift(value & 0x7F);
    value >>= 7;
  }

  while (true) {
    var byte = this.readUint8();
    items.push(byte & 0x7F);
    if (!(byte & 0x80)) break;
  }

  return new BigInt(items, 7);
};
